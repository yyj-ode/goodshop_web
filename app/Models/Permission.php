...
class Permission extends EntrustPermission
{
    ...
    public static function allListData($start, $length, $columns, $order, $key)
    {
        if($key){
            ...
            return self::with('catalog')->offset($start)->limit($length)->orderBy($columns, $order)->get();
        }
    ...
    public static function getPermissionCatalog($id)
    {
        return self::with('catalog')->where('id',$id)->first()->toArray();
    }
    public static function getPermission($id)
    {
        return self::with('catalog')->where('show',1)->whereIn('id',$id)->get()->toArray();
    }
}